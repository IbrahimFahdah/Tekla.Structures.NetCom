...
namespace Exercise
{
    public partial class Form1 : Form
    {
        ...
        private void CreateRebars()
        {
            if (MyModel.GetConnectionStatus())
            {
                IModelObjectEnumerator ObjectsInModel = MyModel.GetModelObjectSelector().GetAllObjects();

                while (ObjectsInModel.MoveNext())
                {
                    var MyBeam = ObjectsInModel.Current as IBeam;
                    ...
                    if (MyBeam != null && MyBeam.Name == "FOOTING") //use same name as given in exercise 1
                    {
                        ...
                        IModelObjectEnumerator BeamChildren = MyBeam.GetChildren();
                        bool HasRebars = false;

                        while (BeamChildren.MoveNext())
                        {
                            if (BeamChildren.Current is IReinforcement)
                            {
                                HasRebars = true;
                            }
                        }
        ...
        /// <returns></returns>
        private void CreateRebar(IModelObject PadFooting, double PositionX, double PositionY)
        {
            IRebarGroup Rebar = MyModel.CreateRebarGroup();
            IPolygon RebarPolygon1 = MyModel.CreatePolygon();
            double MyFootingSize = double.Parse(FootingSize.Text);

            //use given position and footing size
            RebarPolygon1.Points.Add(MyModel.CreatePoint(PositionX - MyFootingSize / 2.0, PositionY - MyFootingSize / 2.0, 0));
            RebarPolygon1.Points.Add(MyModel.CreatePoint(PositionX + MyFootingSize / 2.0, PositionY - MyFootingSize / 2.0, 0));
            RebarPolygon1.Points.Add(MyModel.CreatePoint(PositionX + MyFootingSize / 2.0, PositionY + MyFootingSize / 2.0, 0));
            RebarPolygon1.Points.Add(MyModel.CreatePoint(PositionX - MyFootingSize / 2.0, PositionY + MyFootingSize / 2.0, 0));
            RebarPolygon1.Points.Add(MyModel.CreatePoint(PositionX - MyFootingSize / 2.0, PositionY - MyFootingSize / 2.0, 0));
            ...
            Rebar.EndPointOffsetType = RebarOffsetTypeEnum.OFFSET_TYPE_COVER_THICKNESS;
            Rebar.EndPointOffsetValue = 20.0;
            Rebar.StartPointOffsetType = RebarOffsetTypeEnum.OFFSET_TYPE_COVER_THICKNESS;
            ...
            Rebar.SpacingType = RebarGroupSpacingTypeEnum.SPACING_TYPE_TARGET_SPACE;
            Rebar.Spacings.Add(100.0);
            Rebar.ExcludeType = ExcludeTypeEnum.EXCLUDE_TYPE_BOTH;
            ...
            Rebar.StartHook.Shape = RebarHookShapeEnum.HOOK_90_DEGREES;
            Rebar.EndHook.Shape = RebarHookShapeEnum.HOOK_90_DEGREES;
        ...
        /// <returns></returns>
        private IPolygon GetPolygonBySolidsAABB(IPart InputObject)
        {
            IPolygon Result = MyModel.CreatePolygon();

            ISolid BeamSolid = InputObject.GetSolid();
            Result.Points.Add(MyModel.CreatePoint(BeamSolid.MaximumPoint.X, BeamSolid.MaximumPoint.Y, 0));
            Result.Points.Add(MyModel.CreatePoint(BeamSolid.MinimumPoint.X, BeamSolid.MaximumPoint.Y, 0));
            Result.Points.Add(MyModel.CreatePoint(BeamSolid.MinimumPoint.X, BeamSolid.MinimumPoint.Y, 0));
            Result.Points.Add(MyModel.CreatePoint(BeamSolid.MaximumPoint.X, BeamSolid.MinimumPoint.Y, 0));
            Result.Points.Add(MyModel.CreatePoint(BeamSolid.MaximumPoint.X, BeamSolid.MaximumPoint.Y, 0));
        ...
        {
            IModelObject PadFooting = CreatePadFooting(PositionX, PositionY, double.Parse(FootingSize.Text));
            IModelObject Column = CreateColumn(PositionX, PositionY);
        ...
        /// <returns></returns>
        private IModelObject CreatePadFooting(double PositionX, double PositionY, double FootingSize)
        {
            IBeam PadFooting = MyModel.CreateBeam();
            ...
            PadFooting.Position.Rotation = RotationEnum.FRONT;
            PadFooting.Position.Plane = PlaneEnum.MIDDLE;
            PadFooting.Position.Depth = DepthEnum.MIDDLE;
        ...
        /// <param name="PositionY">Y-coordination of the position</param>
        /// <returns></returns>
        private IModelObject CreateColumn(double PositionX, double PositionY)
        {
            IBeam Column = MyModel.CreateBeam();
            ...
            Column.Position.Rotation = RotationEnum.FRONT;
            Column.Position.Plane = PlaneEnum.MIDDLE;
            Column.Position.Depth = DepthEnum.MIDDLE;
        ...
        private void CreateBasePlate(IModelObject PrimaryObject, IModelObject SecondaryObject)
        {
            IConnection BasePlate = MyModel.CreateConnection();

            BasePlate.Name = "Stiffened Base Plate";
            BasePlate.Number = 1014;
            BasePlate.LoadAttributesFromFile("standard");
            BasePlate.UpVector = MyModel.CreateVector(0, 0, 1000);
